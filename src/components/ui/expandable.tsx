...
interface ExpandableContextType {
  ...
  easeType: "easeInOut" | "linear" | "easeIn" | "easeOut" // Easing function for the animation
  initialDelay: number // Delay before the animation starts
  expandMode: "click" | "hover" // How the expansion is triggered
...
const ExpandableContext = createContext<ExpandableContextType>({
  ...
  initialDelay: 0,
  expandMode: "click", // Default expand mode
})
...
interface ExpandableProps extends ExpandablePropsBase {
  ...
  easeType?: "easeInOut" | "linear" | "easeIn" | "easeOut" 
...
const ExpandableCard = React.forwardRef<HTMLDivElement, ExpandableCardProps>(
  (
    ...
    const { isExpanded, toggleExpand, expandDirection, expandMode } = useExpandable()
    ...
    const handleHover = () => {
      if (expandMode === "hover" && !isExpanded) {
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-1 rounded-lg sm:rounded-xl md:rounded-[2rem] p-1 sm:p-1.5 md:p-2 shadow-md shadow-black/5">
            <div className="rounded-md sm:rounded-lg md:rounded-3xl bg-[#f8f8ff] dark:bg-zinc-900 p-2 sm:p-3 md:p-4 shadow-xl ring-1 ring-black/5">
...
>(({ children, ...props }, ref) => {
  const { toggleExpand, expandMode } = useExpandable()
  return (
    <div
      ref={ref}
      onClick={expandMode === "click" ? toggleExpand : undefined}
